...
struct chardev_t {
  ...
  struct page *page;       // alloc_page gets this
  void *addr;              // vaddr of page
...
ssize_t _write(struct file *f, const char __user *buf, size_t count, 
  ...
  mc = copy_from_user(c.addr, buf, sz);
  if (mc == 0) c.bytes_available = sz;
  mutex_unlock(&mutex);
...
int __init chardev_init(void) {
  ...
  c.page = alloc_page(GFP_KERNEL);
  if (!c.page) {rc = -ENOMEM; goto done; }
  c.addr = page_address(c.page);
  if (!c.addr) {
    rc = -ENOMEM;
    printk(KERN_DEBUG "page_address failed\n");
    __free_page(c.page);
    goto done;
  }
